...
public class PhieuMuonDAO {
    ...
    public boolean thaydoiTThai(String maPM){
        ...
        contentValues.put("TrangThai", "Da tra");
    ...
    //Cập nhật phiếu mượn
    public boolean SuaPMuon(PhieuMuon phieuMuon){
        SQLiteDatabase sqLiteDatabase = myDbHelper.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put("maPhieu", phieuMuon.getMaPM());
        contentValues.put("maTV", phieuMuon.getMaTV());
        ...
        long check = sqLiteDatabase.update("PhieuMuon", contentValues, "maPhieu=?", new String[]{phieuMuon.getMaPM()});
        if (check == -1){
            return false;
        }
        return true;
